...
Ziggurat::Ziggurat(int number_of_rects, double s, int tailfactor, int precision)
	: omega(precision), m(ConvPrec(number_of_rects, omega)),
	  sigma(ConvPrec(s, precision)), tailcut(to_RR(sigma * tailfactor)),
	...
	if (m == 1) {
		...
		// m != 1
		// increase right bound until reaching 14*sigma and compute possible
		// partition(s)
		while (tailcut < to_RR((tailfactor + 1) * sigma)) {
			...
			while (difference < 0 || (abs(difference) > prec &&
									  abs(difference - lastdiff) > prec)) {
				cc = c;
				lastdiff = difference;
				difference =
					compute_recurrence(xis, rhos, c, m, sigma, tailfactor) -
					to_RR(1);
				if (difference == -2) {
			...
			// if while-loop did not improve anything, increase tailcut and go
			// to next iteration
			if (difference < 0 ||
	...
	if (bestxis[number_of_rects] != -1) {
		// output precision, number of rectangles m, (std. deviation) sigma, and
		// y0>=1 (the value for x=0)
		y0 = to_RR(1) + bestdiff;
	...
	while (true) {
		...
		if (x != 0 && x <= rectxs[i - 1]) {
			...
			if (x == 0) {
				...
				// y <- [0,\floor{2^{\omega} (\rho_{\sigma}(x_{i-1}) -
				// \rho_{\sigma}(x_{i}))}]
				if (y <= TruncToZZ(to_RR(yfactor) *
								   (rho(to_RR(x), sigma) - rectys[i]))) {
					break;
				}
			}
		}
...
RR compute_recurrence(RR *xis, RR *rhos, RR c, RR m, RR sigma, int tailfactor) {
	...
	// manually set y-value for largest x-value (rounded to the next-largest
	// integer)
	...
	for (int i = em - 3; i > 0; i--) {
		sqrtv = m2 * log(area / to_RR(TruncToZZ(xis[i + 1]) + to_ZZ(1)) +
						 rho(xis[i + 1], sigma));
		if (sqrtv < to_RR(0)) {
			return to_RR(-1);
		}
