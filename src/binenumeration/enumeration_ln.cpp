...
vector<long> LengthPruningOpt(matrix<long> const& B, vector<double> const& rbound,
		vector<double> const& t, long q, matrix<double> const& mu)
{
	// initialize
	size_t m = B.size();
	...
	for (size_t k=m; k>0; --k)
	{
		...
		{
			sigma[i-1][k-1] = sigma[i][k-1] + (t[i-1] - v[i-1]) * mu[i-1][k-1];
		}
	...
	while (true) {
		...
		if (rho[k] <= rbound[k]) // we generate the sequence of the form R_1^2 > ... > R_m^2
		{
			if (k==0)
			{
...
vector<long> LengthPruningOptParall(matrix<long> const& B,
		matrix<double> const& B_star, vector<double> const& rbound, vector<long> const& t,
		long q, size_t lvl)
{
	...
	while (!got_sigterm)
	{
		...
		double interval = sqrt(fabs(rbound[m-j-1] - errVec[j]) / mu[m-j-1][m-j-1]);
			...
			while (c[j] <= c_max[j])
			{
				...
				{   // lock mutex for thread count, wait until we can spawn
					...
					future_solutions.push_back(async(launch::async,
						LengthPruningOptThread, solutions.back(), B, mu, rbound, t_coeff,
						c, j+1, errVec_new));
					number_of_threads++;
