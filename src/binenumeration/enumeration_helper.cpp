	...
	long q, double factor)
{
	...
	for (size_t i = 0; i < m; ++i)
	{
		tmp = 3 * s * factor / (2 * sqrt( A_mu[i][i] ) ); // 3 might be reduced/changed --EK
		if (tmp < 1)
			d[i] = 1;
		else
			d[i] = (long) ceil(tmp);
 ...
 */
vector<long> ComputeD_binary(matrix<double> A_mu, double s, int n, double factor, double factor_bin)
{
	size_t m = A_mu.size();
	vector<long> d(m);
	double tmp;
...
vector<double> ComputeR_LP(matrix<double> A_mu, vector<long> d)
{
	...
	{
		R[m-i] = R[m-i+1]+(d[m-i]) * (d[m-i]) * A_mu[m-i][m-i]/6;
		//cout << A_mu[i-1][i-1] << " ";
	}
	cout << endl;
	return R;
 ...
 * \brief TODO
 */
vector<double> ComputeRlength(matrix<double> A_mu, double s, double factor, double babaiBound)
{
	size_t m = A_mu.size();
	vector<double> R(m);
	for (size_t i = 1; i <= m; ++i)
...
/**
 * ComputeLvlNP
 * \brief computes the level from where to start parallel runs - this is easy in the
 *        Lindner Peikert case, as the d vector codes the number of iterations at each
 *        level in the search tree. Thus we can just multiply d's elements, until we
 *        reach the number of desired threads
 *
 * @params d Lindner Peikerts d vector
 * @params n_threads if we want to run less threads than available by the hardware
 */
size_t ComputeLvlNP(vector<long> const& d, int n_threads)
{
	...
	// compute which level in the iteration tree has enough paths for spawning threads
	size_t lvl = 0;
	long num_threads = 1;
	...
	cout << lvl << endl;
	cout << "number of threads to start at this level: ";
 ...
 * @params n_threads if we want to run less threads than available by the hardware
 */
size_t ComputeLvlLength(matrix<long> const& B, vector<double> const& R, vector<long> t,
		int n_threads)
{
	// check if we got a command line argument to bound the number of threads
	if ((n_threads > 0) && (n_threads <= number_of_max_threads))
	...
	for (size_t i=R.size()-1; i>0; i--)
	{
		...
		double interval = sqrt(fabs((R[m-level-1] - errL2)) / B_star_len[m-level-1]);
		double c_star = inner_product(t.cbegin(), t.cend(), B_star_orth[m-level-1].cbegin(), 0.0);
